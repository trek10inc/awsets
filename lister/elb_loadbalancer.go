...
func (l AWSElbLoadbalancer) List(cfg option.AWSetsConfig) (*resource.Group, error) {
	svc := elasticloadbalancing.NewFromConfig(cfg.AWSCfg)

	rg := resource.NewGroup()
	err := Paginator(func(nt *string) (*string, error) {
		res, err := svc.DescribeLoadBalancers(cfg.Context, &elasticloadbalancing.DescribeLoadBalancersInput{
			Marker: nt,
		})
		if err != nil {
			return nil, err
		...
		for _, v := range res.LoadBalancerDescriptions {
			r := resource.New(cfg, resource.ElbLoadBalancer, v.LoadBalancerName, v.LoadBalancerName, v)
