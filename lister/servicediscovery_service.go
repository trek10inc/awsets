...
func (l AWSServiceDiscoveryService) List(cfg option.AWSetsConfig) (*resource.Group, error) {
	svc := servicediscovery.NewFromConfig(cfg.AWSCfg)

	rg := resource.NewGroup()
	err := Paginator(func(nt *string) (*string, error) {
		res, err := svc.ListServices(cfg.Context, &servicediscovery.ListServicesInput{
		...
		for _, service := range res.Services {
			v, err := svc.GetService(cfg.Context, &servicediscovery.GetServiceInput{
				Id: service.Id,
			})
			if err != nil {
				return nil, fmt.Errorf("failed to describe service discovery service %s: %w", *service.Id, err)
			}
			r := resource.New(cfg, resource.ServiceDiscoveryService, v.Service.Id, v.Service.Name, v.Service)
			r.AddRelation(resource.ServiceDiscoveryNamespace, v.Service.NamespaceId, "")

			// Service Discovery Instances
			err = Paginator(func(nt2 *string) (*string, error) {
				instances, err := svc.ListInstances(cfg.Context, &servicediscovery.ListInstancesInput{
				...
				for _, instance := range instances.Instances {
					instanceR := resource.New(cfg, resource.ServiceDiscoveryInstance, instance.Id, instance.Id, instance)
